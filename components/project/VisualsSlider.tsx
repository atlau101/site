...
export const VisualsSlider: React.FC<VisualsSliderProps> = ({ visuals }) => {
  ...
  const handleTouchEnd = (e: React.TouchEvent<HTMLDivElement>) => {
    ...
    if (deltaX > 0 && idx > 0) prev();
    if (deltaX < 0 && idx < visuals.length - 1) next();
  };
    ...
    >
      ...
      <div className="flex items-center justify-center gap-2 sm:gap-6">
        <button
          onClick={prev}
          disabled={idx === 0}
          className="flex h-11 w-11 shrink-0 touch-manipulation items-center justify-center text-muted-foreground transition-colors hover:text-foreground active:bg-muted active:text-foreground disabled:pointer-events-none disabled:opacity-25"
        ...
        <div
          className="relative min-w-0 flex-1 touch-pan-y overflow-hidden bg-paper-dim aspect-video"
